...
// FIXME: should become "explicit" some time
    ...
    pointing (const vec3 &inp)
      ...
      phi = ((inp.y==0.) && (inp.x==0.)) ? 0.0 : atan2(inp.y,inp.x);
...
// FIXME: should be removed some time
    ...
    void normalize()
      {
      //theta=fmodulo(theta,2.0*M_PI);
      theta = (theta>=0) ? ((theta<2.0*M_PI) ? theta : fmod(theta,2.0*M_PI)) : (fmod(theta,2.0*M_PI)+2.0*M_PI);
      if (theta>M_PI)
        {
        phi+=M_PI;
        theta=2.0*M_PI-theta;
        }
      //phi=fmodulo(phi,2.0*M_PI);
      phi = (phi>=0) ? ((phi<2.0*M_PI) ? phi : fmod(phi,2.0*M_PI)) : (fmod(phi,2.0*M_PI)+2.0*M_PI);
      }
  };
...
inline void vec2pnt(const vec3 &vec, pointing &ptg)
  ...
  ptg.phi = ((vec.y==0.) && (vec.x==0.)) ? 0.0 : atan2(vec.y,vec.x);
  if (ptg.phi<0) ptg.phi += 2.0*M_PI;
  }
